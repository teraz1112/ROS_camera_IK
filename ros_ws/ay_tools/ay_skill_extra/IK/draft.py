...
def MakeCameraAxis_Rand(range,roll,pitch,yaw,euler):
   ...
   camera=np.zeros(2)
   p=np.dot(R,euler[:3])+dis
   random_number1 = random.uniform(-0.0005, 0.0005)
   random_number2 = random.uniform(-0.0005, 0.0005)
...
def Run(ct,*args):
    ...
    for i in tqdm(range(500), desc="Processing", unit="iteration"):
        ...
        calibpoint.append(g_tp1)
        calibpoint.append(g_tp2)
        calibpoint.append(g_tp3)
        calibpoint.append(g_tp4)
        calibpoint.append(g_tp5)
        calibpoint.append(g_tp6)

        for k in calibpoint:
            m=MakeCameraAxis_Rand(t_miss,roll1_miss,pitch1_miss,yaw1_miss,k)
            camera.append(m)
            A.append(np.array([k[0],k[1],k[2] , -m[0]*k[0],-m[0]*k[1],-m[0]*k[2],0,0,0,1,0]))
            A.append(np.array([0,0,0,-m[1]*k[0],-m[1]*k[1],-m[1]*k[2],k[0],k[1],k[2] , 0,1]))
            xc.append(m*4)
        reshaped_A = np.reshape(A, (-1, 11))
        reshaped_xc= np.reshape(xc, (-1, 1))
        solution = LST(reshaped_A,reshaped_xc)
        
        R_guess=np.reshape(np.array([[solution[0],solution[1],solution[2]],
                            [solution[3],solution[4],solution[5]],
                            [solution[6],solution[7],solution[8]],]),(3,3))
